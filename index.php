...
$event = \mod_stepbystep\event\course_module_instance_list_viewed::create($params);
$event->add_record_snapshot('course', $course);
$event->trigger();

$strstep = get_string('modulename', 'mod_stepbystep');
$strsteps = get_string('modulename', 'mod_stepbystep');
$strname = get_string('name');
$strintro = get_string('moduleintro');
$strlastmodified = get_string('lastmodified');

$PAGE->set_title($course->shortname . ': ' . $strsteps);
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add($strsteps);
echo $OUTPUT->header();
echo $OUTPUT->heading($strsteps);

if (!$stepbysteps = get_all_instances_in_course('stepbystep', $course)) {
    notice(get_string('thereareno', 'moodle', $strsteps), "$CFG->wwwroot/course/view.php?id=$course->id");
    exit;
}
...
foreach ($stepbysteps as $stepbystep) {
    ...
    $table->data[] = array(
        ...
        format_module_intro('stepbystep', $stepbystep, $cm->id));
}
